...
class VerifyRepository(private val api: AuthApi) : VerifyContract.Model, ThreadsHelper() {
    private var storage = LocalStorage.instance

    override fun verify(verifyData: VerifyData, block: (ResultData<String>) -> Unit) {
        api.verify(verifyData).enqueue(object : Callback<ResponseData<String>> {
